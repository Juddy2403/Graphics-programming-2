...
static bool ParseOBJ(const std::string& filename, std::vector<Vertex3D>& vertices, std::vector<uint16_t>& indices, bool flipAxisAndWinding = true)
{
	...
	for (auto& v : vertices)
	{
		v.m_Tangent = glm::normalize(v.m_Tangent - glm::dot(v.m_Tangent, v.m_Normal) * v.m_Normal);
